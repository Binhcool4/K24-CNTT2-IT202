...
DELETE FROM likes WHERE post_id IN (101,102,103,104);
...
delimiter //
create procedure CreatePostWithValidation(in p_user_id int ,in p_content text, out error_mess text)
begin 
	 declare total_post int default 0;
     select count(post_id) into total_post from posts where user_id = p_user_id;
	
    if CHAR_LENGTH(p_content)  <  5 then
			set error_mess = 'Số lượng kí tự quá ngắn';
	else
		set error_mess = '';
            end if;
end //
...
set @error_validate = '';
call CreatePostWithValidation(1,'hung dep trai',@error_validate);
