...
begin 
	 declare total_point int default 0;
     
    set total_point = 
    ((select count(post_id)  from posts where user_id = p_user_id) * 10) +
    ((select count(post_id) from comments where user_id = p_user_id) * 5) + 
    ((select count(post_id)  from likes where user_id = p_user_id) * 3);
    if total_point > 500 then
			set activity_level  = 'Rất tích cực';
	elseif total_point in(200,500) then
			set activity_level  = 'Tích cực';
	elseif total_point < 200 then
			set activity_level  = 'Bình thường';
            end if;
end //


call CalculateUserActivityScore(1,@point,@level);
SELECT @level;
