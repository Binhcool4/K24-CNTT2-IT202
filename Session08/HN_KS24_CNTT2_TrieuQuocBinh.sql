...
-- 4.Liệt kê các lần đặt phòng diễn ra trong năm 2024
select * from bookings
where year(check_in) = 2024;

-- 5.Cho biết số lượng phòng của từng loại phòng
select room_id, count(*) as so_luong_phong from bookings
group by room_id;
...
select g.guest_id, g.guest_name, count(b.booking_id) as so_lan_dat_phong from guests g
...
select r.room_id, r.room_type, COUNT(*) * r.price_per_day as doanh_thu from rooms r
join bookings b on r.room_id = b.room_id
group by r.room_id, r.room_type, r.price_per_day;

-- 4.Tìm những khách đã đặt phòng từ 2 lần trở lên
...
-- 5.Tìm loại phòng có số lượt đặt phòng nhiều nhất
