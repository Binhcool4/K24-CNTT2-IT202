...
SELECT 
    ...
    SUM(oi.quantity) AS `Tổng số lượng đã bán`, -- Hiển thị mỗi sản phẩm đã bán được bao nhiêu sản phẩm
    SUM(oi.quantity * p.price) AS `Doanh thu` -- Tính doanh thu của từng sản phẩm
FROM products p
